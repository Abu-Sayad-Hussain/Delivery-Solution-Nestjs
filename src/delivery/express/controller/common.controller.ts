...
import { ExpressCommonService } from '../service/common.service';
...
@Controller('api/v1.0.0/delivery/express')
@UseInterceptors(ResponseInterceptor)
export class ExpressCommonController {
  constructor(
    private readonly amqpConnection: AmqpConnection,
    private readonly expressCommonService: ExpressCommonService
  ) {}
  ...
  async create(@Body() createDeliveryDto: DeliveryDto) {
    try {
      const delivery = await this.expressCommonService.create(
        createDeliveryDto,
      );
      if (!delivery){
  ...
  async findAll(@Query() deliveryQuery: DeliveryQueryDto) {
    try {
      const delivery = await this.expressCommonService.findAll(deliveryQuery);
      if (!delivery) {
  ...
  public async findByDeliveryId(@Param('delivery_id') deliveryId: string) {
    try {
      const delivery = await this.expressCommonService.findByDeliveryId(
        deliveryId,
      );
      if (!delivery) {
        throw new NotFoundException('Delivery does not exist!');
  ...
  public async updateByDeliveryId(
    ...
    try {
      ...
      const delivery = await this.expressCommonService.updateByDeliveryId(
        deliveryId,
        updateDeliveryDto,
      );
      if (!delivery) {
  ...
  public async updateStatusByOrderId(
    ...
    try {
      ...
      const delivery = await this.expressCommonService.updateStatusByOrderId(updateDeliveryDto);
      if (!delivery) {
        throw new NotFoundException('Delivery does not exist!');
      }
      return {
  ...
  public async heroActiveOrderByHeroId(
    ...
    try {
      const delivery = await this.expressCommonService.heroActiveOrderByHeroId(heroId, deliveryQuery);
      if (!delivery) {
        throw new NotFoundException('Delivery does not exist!');
      }
      return {
